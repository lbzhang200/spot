...
from spotify_client import (
    get_auth_url,
    get_spotify_from_code,
    get_spotify_from_session,
    get_current_user,
    get_top_tracks,
    get_top_artists,
    get_recently_played,
...
def _require_auth():
    """
    Cloud-compatible OAuth flow:
    1. Check if we already have a token in session state → use it
    2. Check if Spotify redirected back with ?code=... → exchange it
    3. Otherwise show a Login button
    """
    # Step 1: already logged in
    sp = get_spotify_from_session(REDIRECT_URI, st.session_state)
    if sp:
        return sp, get_current_user(sp)

    # Step 2: returning from Spotify OAuth redirect
    params = st.query_params
    code = params.get("code")
    if code:
        try:
            sp = get_spotify_from_code(code, REDIRECT_URI, st.session_state)
            st.query_params.clear()
            return sp, get_current_user(sp)
        except Exception as e:
            st.error(f"Login failed: {e}")
            st.session_state.pop("_spotify_token", None)
    ...
    st.markdown(
        ...
        <div style="display:flex;flex-direction:column;align-items:center;
            ...
            <h1 style="color:#FFFFFF;margin:0">Spotify Mood Analyzer</h1>
            <p style="color:#b3b3b3;font-size:16px;max-width:400px">
                Analyze your listening history, discover your mood clusters,
                and generate new playlists.
            </p>
        </div>
        """,
        unsafe_allow_html=True,
